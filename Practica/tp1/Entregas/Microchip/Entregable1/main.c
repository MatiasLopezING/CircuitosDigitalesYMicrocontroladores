...
// Env�a un solo byte (8 bits) al Neopixel
static inline void neopixel_enviarByte(uint8_t byte) {
...
Hay que tener en cuenta que luego de poner en bajo el bit, hay desplazamiento, bucle for, y asignacion para volver a subir la se�al. Todo esto agrega overhead pero al nosotros ya
...
Para nuestra solucion pusimos una cantidad de NOP's que mantenga la se�al en bajo o en alto un tiempo cercano al que marca el datasheet, debido al overhead aumentara ese tiempo pero sin hacer que deje de funcionar la transmision.
*/

// Env�a un color completo (24 bits) a un LED
static inline void neopixel_enviarColor(uint8_t rojo, uint8_t verde, uint8_t azul) {
...
static inline void secuenciaC() {
	...
	_delay_us(60); // Tiempo de reset para los Neopixels (m�s de 50us)
}
